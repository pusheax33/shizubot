import aiohttp
import json
from bot_vars import BIN_API_KEY, BIN_ENDPOINT
...
class ImageSearch:
    ...
    headers = {"Ocp-Apim-Subscription-Key": BIN_API_KEY,
...
class ShizuImage:
    ...
    # https://shizu.cognitiveservices.azure.com/bing/v7.0
    search_term = "gatitos"
    ...
    async def img(self, message, imageType = None):
        ...
        params = {"q": img_to_search, "safeSearch": "Off", "imageType": imageType}
        if not imageType:
            params = {"q": img_to_search, "safeSearch" : "Off"}

        response = await self.image_search.fetch_json(BIN_ENDPOINT + "/images/search", params)
        if not response:
    ...
    async def gif(self, message):
        message.content = ";img " + message.content
